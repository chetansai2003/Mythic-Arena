...
export function createApp({
  ...
  app.get('/health/ready', async (req, res) => {
    ...
    const ready = status.redis && status.mongo && (!services || await services.isReady());
    res
  ...
  app.use((error, req, res, next) => {
    ...
    if (error instanceof HttpError) return res.status(error.status).json({ error: { code: error.code, message: error.message, requestId: req.requestId, ...(error.details ? { details: error.details } : {}) } });
    const invalid =
    ...
    const dependencyError = /Mongo|Redis|Socket|ClientClosed/.test(error.name ?? '');
    res.status(invalid ? 400 : dependencyError ? 503 : 500).json({
      error: {
        code: invalid ? 'INVALID_PAYLOAD' : dependencyError ? 'DEPENDENCY_UNAVAILABLE' : 'INTERNAL_ERROR',
        message: invalid
