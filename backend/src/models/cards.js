...
const unit = (id, name, faction, cost, attack, health, keywords = []) => ({
  id,
  name,
  faction,
  cost,
  ...
  rarity: keywords.length ? 'RARE' : 'COMMON',
  artRef: null,
});
const spell = (id, name, faction, cost, key, amount) => ({
  id,
  name,
  faction,
  cost,
  ...
  version: 1,
  rulesVersion: RULES_VERSION,
  rarity: key === 'SHIELD' ? 'RARE' : 'COMMON',
  artRef: null,
  effect:
    key === 'SHIELD'
      ? { key, target: 'FRIENDLY_CHARACTER' }
      : {
          key,
          amount,
          target: key === 'DAMAGE' ? 'ENEMY_CHARACTER' : 'FRIENDLY_CHARACTER',
        },
});
export const CATALOG = [
  unit('frostwatch_sentinel', 'Frostwatch Sentinel', 'NORSE', 2, 1, 4, [
    'GUARD',
  ]),
  ...
  cardDefinitionSchema.parse(card);
  assertDeclaredBehavior(card);
