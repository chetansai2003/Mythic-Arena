...
export async function createAuthService({
  ...
  const families = db.collection('session_families');
  ...
  async function access(user, familyId) {
    ...
    const expiry = issuedAt + config.ACCESS_TOKEN_SECONDS;
    const accessToken = await new SignJWT({ sid: familyId })
      .setProtectedHeader({ alg: 'HS256' })
      .setSubject(user._id)
      .setIssuer('mythic-arena')
      .setAudience('mythic-web')
  ...
  async function newSession(user, session) {
    ...
    return {
      ...(await access(user, familyId)),
      refreshToken,
      refreshExpiresAt: expiresAt,
    };
  }
  async function notifyRevoked(familyId) {
    try {
      await onSessionRevoked(familyId);
    } catch {
      /* Revocation is already durable; transport notification must not undo it. */
    }
  }
  return {
      ...
      try {
        ...
        if (error.code === 11000)
          throw new HttpError(
            409,
            'ACCOUNT_UNAVAILABLE',
            'An account could not be created with those details. Try signing in.',
          );
        throw error;
      }
    },
    ...
    async refresh(rawToken) {
      if (typeof rawToken !== 'string' || !/^[a-zA-Z0-9_-]{64}$/.test(rawToken))
      ...
      const result = await transaction(async (session) => {
        ...
        if (record.usedAt) {
          await families.updateOne(
            { _id: family._id },
            { $set: { revokedAt: new Date(now()), reason: 'TOKEN_REUSE' } },
            { session },
          );
          return { reused: family._id };
        }
        ...
        return {
          ...(await access(user, family._id)),
          refreshToken,
          refreshExpiresAt: family.expiresAt,
        };
      });
      if (result.reused) {
        await notifyRevoked(result.reused);
        throw invalidSession();
      }
      return result;
    },
    async authenticate(authorization) {
      ...
      let payload;
      try {
        ({ payload } = await jwtVerify(authorization.slice(7), key, {
          issuer: 'mythic-arena',
          audience: 'mythic-web',
          algorithms: ['HS256'],
    ...
    async logout(rawToken, authorization) {
      ...
      if (typeof rawToken === 'string') {
        ...
        if (record) familyIds.add(record.familyId);
      }
      if (
        typeof authorization === 'string' &&
        authorization.startsWith('Bearer ') &&
        ...
        try {
          ...
          if (typeof payload.sid === 'string') familyIds.add(payload.sid);
        } catch {
          /* An expired access token does not prevent cookie-session logout. */
        }
      }
      for (const familyId of familyIds) {
        await families.updateOne(
          { _id: familyId, revokedAt: null },
          { $set: { revokedAt: new Date(now()), reason: 'LOGOUT' } },
        );
        await notifyRevoked(familyId);
      }
    },
  };
}
