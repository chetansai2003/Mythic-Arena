...
import { HttpError, parseInput } from '../utils/errors.js';

export function createDeckService(db, now = Date.now) {
  const decks = db.collection('decks');
  const view = (deck) =>
    savedDeckSchema.parse({
      id: deck._id,
      name: deck.name,
      cardIds: deck.cardIds,
      revision: deck.revision,
      rulesVersion: deck.rulesVersion,
  ...
  async function catalog() {
    const docs = await db
      .collection('cards')
      .find({ catalogVersion: 1 })
      .sort({
        'definition.faction': 1,
        'definition.cost': 1,
        'definition.name': 1,
      })
      .toArray();
    return docs.map((doc) => {
      const definition = cardDefinitionSchema.parse(doc.definition);
      assertDeclaredBehavior(definition);
      return definition;
    });
  }
  async function validate(input) {
    const data = parseInput(playableDeckSchema, input);
    const definitions = await catalog();
    const ids = new Set(definitions.map((card) => card.id));
    if (data.cardIds.some((id) => !ids.has(id)))
      throw new HttpError(
        400,
        'INVALID_DECK',
        'This deck contains a card that is not in the catalog.',
      );
    return data;
  }
  async function owned(userId, id) {
    const deck = await decks.findOne({ _id: id, userId });
  ...
  return {
    ...
    async list(userId) {
      return (
        await decks.find({ userId }).sort({ updatedAt: -1, _id: 1 }).toArray()
      ).map(view);
    },
    ...
    async update(userId, id, input) {
      ...
      const data = await validate({
        ...
        cardIds: input.cardIds ?? old.cardIds,
      });
      const next = await decks.findOneAndUpdate(
        { _id: id, userId, revision: input.expectedRevision },
        {
      ...
      if (!next)
        throw new HttpError(
          409,
          'DECK_CONFLICT',
          'This deck changed in another tab. Your edits are preserved; reload the saved version or save a copy.',
        );
      return view(next);
    },
    async remove(userId, id, expectedRevision) {
      ...
      if (!result.deletedCount)
        throw new HttpError(
          409,
          'DECK_CONFLICT',
          'This deck changed. Reload it before deleting.',
        );
    },
    async snapshotForMatch(userId, id) {
      const deck = await owned(userId, id);
      const definitions = await catalog();
      ...
      return structuredClone({
        ...
        rulesVersion: deck.rulesVersion,
        catalogVersion: deck.catalogVersion,
        cards: deck.cardIds.map((definitionId) => ({
