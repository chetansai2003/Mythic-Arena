...
import { damage, determineOutcome, findTarget, finish, nextTurn, removeDeaths } from './rules.js';
import { EFFECT_HANDLERS } from './registry.js';

function event(state, type, now, actionId = null) {
  return { eventId: `event_${state.version}`, gameId: state.gameId, version: state.version, actionId, type, occurredAt: now };
}
...
export function advanceTurn(state, now) {
  ...
  next.version++;
  next.updatedAt = now;
  return { state: next, events: [event(next, next.outcome ? 'MATCH_ENDED' : 'TURN_STARTED', now)] };
}
...
export function applyAction(state, actorId, input, now) {
  ...
  if (action.type === 'SURRENDER') {
    ...
    const index = player.hand.findIndex((c) => c.instanceId === action.payload.cardInstanceId);
    const card = player.hand.splice(index, 1)[0];
    const definition = card.definition;
    player.energy -= definition.cost;
    if (definition.kind === 'UNIT') {
      player.board.push({
        instanceId: card.instanceId, definitionId: definition.id, definition,
        attack: definition.attack, health: definition.health, maxHealth: definition.health,
        guard: definition.keywords.includes('GUARD'), shield: definition.keywords.includes('SHIELD'), canAttack: false,
      });
    } else {
      EFFECT_HANDLERS[definition.effect.key](findTarget(next, action.payload.target).character, definition.effect);
      player.discard.push(card);
    }
    type = 'CARD_PLAYED';
  } else {
    const attacker = player.board.find((u) => u.instanceId === action.payload.attackerId);
    const target = findTarget(next, action.payload.target).character;
    const retaliation = action.payload.target.kind === 'UNIT' ? target.attack : 0;
  ...
  next.version++;
  next.updatedAt = now;
  return { ok: true, state: next, events: [...elapsed.events, event(next, next.outcome ? 'MATCH_ENDED' : type, now, action.actionId)] };
}
