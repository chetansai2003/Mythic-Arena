...
import { createMatch, applyAction, advanceTime, advanceTurn, chooseBotAction, determineOutcome, legalActions, projectForPlayer, resolveDeaths, validateAction } from '@mythic/game-engine';
import { activeFixture, command, definitions, fixture, hero, instance, targetUnit, unit } from './engine-fixtures.js';

const play = (state, id, target) => applyAction(state, 'alpha', command(state, 'PLAY_CARD', { cardInstanceId: id, ...(target ? { target } : {}) }), 1001);
const attack = (state, target, attackerId = 'wolf') => applyAction(state, 'alpha', command(state, 'ATTACK', { attackerId, target }), 1001);
...
describe('creation and deterministic boundaries', () => {
  it('freezes input copies, shuffles deterministically and initializes rule values', () => {
    ...
    expect(state.players.map((p) => p.hand)).not.toEqual(fixture('different').players.map((p) => p.hand));
    ...
    expect(Object.values(definitions).some((d) => d.name === 'Edited snapshot')).toBe(false);
  });
  it('rejects illegal decks and duplicate instance IDs', () => {
    const players = fixture().players.map((p) => ({ id: p.id, displayName: p.displayName, cards: [...p.hand, ...p.deck] }));
    players[1].cards[0].instanceId = players[0].cards[0].instanceId;
    expect(() => createMatch({ gameId: 'test', seed: 'a', now: 0, players })).toThrow(/unique/);
    players[1].cards.pop();
    expect(() => createMatch({ gameId: 'test', seed: 'a', now: 0, players })).toThrow();
  });
  it.each([
    ['wrong actor', (s) => command(s, 'END_TURN'), 'intruder'],
    ['stale version', (s) => ({ ...command(s, 'END_TURN'), expectedVersion: 99 }), 'alpha'],
    ['unknown fields', (s) => ({ ...command(s, 'END_TURN'), playerId: 'alpha' }), 'alpha'],
    ['wrong game', (s) => ({ ...command(s, 'END_TURN'), gameId: 'other' }), 'alpha'],
    ['unknown hand card', (s) => command(s, 'PLAY_CARD', { cardInstanceId: 'missing' }), 'alpha'],
  ...
  it('resolves a deadline before accepting a command at the exact boundary', () => {
    const state = activeFixture();
    const result = applyAction(state, 'alpha', command(state, 'END_TURN'), state.turn.endsAt);
    expect(result.ok).toBe(false);
  ...
  it('catches up missed deadlines with scheduled times and rejects backwards clocks', () => {
    ...
    expect(result.events.map((e) => e.occurredAt)).toEqual([31000, 61000, 91000]);
...
describe('units, targets and combat', () => {
  it('summons with entry keywords, spends once and enforces entry delay', () => {
    ...
    expect(result.state.players[0].board[0]).toMatchObject({ shield: true, canAttack: false });
  ...
  it('rejects full boards, unaffordable cards and unit targets', () => {
    ...
    state.players[0].board = Array.from({ length: 5 }, (_, n) => unit('aurora_wolf', `slot_${n}`));
  ...
  it('resolves simultaneous unit deaths, conserves both discarded cards and prevents retaliation from heroes', () => {
    ...
    expect(result.state.players.map((p) => p.discard.at(-1).instanceId)).toEqual(['wolf', 'enemy']);
  ...
  it('requires a Guard target for attacks but permits damage spells to bypass it', () => {
    ...
    expect(play(state, 'winter_spark', hero('beta')).state.players[1].health).toBe(17);
  });
  it('consumes shield on positive damage only and applies retaliation despite shield', () => {
    const state = activeFixture();
    state.players[0].board = [unit('aurora_wolf', 'wolf', { attack: 0, shield: true })];
    state.players[1].board = [unit('obsidian_scarab', 'enemy')];
    const result = attack(state, targetUnit('enemy'));
    expect(result.state.players[1].board[0].shield).toBe(true);
    expect(result.state.players[0].board[0]).toMatchObject({ health: 1, shield: false });
  });
...
describe('finite effects and draw rules', () => {
  it.each(['winter_spark', 'hearthsong', 'aegis_of_echoes'])('resolves %s with correct target, cost and discard', (id) => {
    const state = activeFixture();
    ...
    else if (id === 'hearthsong') expect(result.state.players[0].health).toBe(20);
    else expect(result.state.players[0].shield).toBe(true);
  });
  it('heals a unit only to initial max health and shield never stacks', () => {
    const state = activeFixture();
    state.players[0].board = [unit('frostwatch_sentinel', 'hurt', { health: 1 })];
    state.players[0].hand = [instance('hearthsong'), instance('aegis_of_echoes'), instance('lantern_ward')];
  ...
  it('burns overflow without fatigue and ramps energy only for the new turn owner', () => {
    const state = activeFixture();
    state.players[1].hand = Array.from({ length: 10 }, (_, n) => instance('aurora_wolf', `hand_${n}`));
  ...
  it('fatigues independently, consumes hero shield and does not exceed ten energy', () => {
    ...
    expect(state.players[1]).toMatchObject({ fatigue: 1, health: 20, shield: false });
...
describe('outcomes, privacy and full games', () => {
  it('ends on spell lethal and rejects all subsequent actions', () => {
    ...
    expect(result.state.outcome).toEqual({ kind: 'WIN', winnerId: 'alpha', reason: 'DEFEAT' });
    expect(result.state.turn).toBe(null);
    expect(result.events[0].type).toBe('MATCH_ENDED');
    const rejected = applyAction(result.state, 'alpha', command(result.state, 'END_TURN'), 1002);
    expect(rejected.state).toBe(result.state);
    expect(rejected.ok).toBe(false);
  });
  it('allows out-of-turn surrender but never an outsider surrender', () => {
    const state = activeFixture();
    expect(applyAction(state, 'beta', command(state, 'SURRENDER'), 1001).state.outcome.winnerId).toBe('alpha');
    expect(applyAction(state, 'intruder', command(state, 'SURRENDER'), 1001).ok).toBe(false);
  });
  it('resolves fatigue defeat, simultaneous hero defeat and turn 100 without starting 101', () => {
    ...
    state.players.forEach((p) => { p.health = 0; });
    expect(determineOutcome(state)).toEqual({ kind: 'DRAW', reason: 'SIMULTANEOUS_DEFEAT' });
    state.players.forEach((p) => { p.health = 20; });
    state.turn.number = 100;
    expect(advanceTurn(state, 1001).state.outcome).toEqual({ kind: 'DRAW', reason: 'TURN_LIMIT' });
  ...
  it('removes every dead unit without mutating the supplied state', () => {
    const state = fixture();
    state.players[0].board = [unit('aurora_wolf', 'dead', { health: 0 }), unit('aurora_wolf', 'alive')];
  ...
  it('serializes only allowlisted public data and detached own hand', () => {
    ...
    for (const hidden of [...state.players[1].hand, ...state.players[0].deck, ...state.players[1].deck]) expect(wire).not.toContain(hidden.instanceId);
    ...
    expect(() => projectForPlayer(state, 'observer', 1000)).toThrow(/participants/);
  });
    ...
    function run() {
      ...
      for (let n = 0; n < 1500 && state.status === 'ACTIVE'; n++) {
        ...
        for (const player of state.players) {
          ...
          expect(player.deck.length + player.hand.length + player.board.length + player.discard.length).toBe(30);
